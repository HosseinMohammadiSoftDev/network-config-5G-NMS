...
start_tshark_for_interface() {
    ...
    sudo rm -f "$output_file" 2>/dev/null
    ...
    if [ -n "$filters" ]; then
        ...
        nohup sudo "$TSHARK_PATH" -i "$interface" -f "$filters" -w "$output_file" > /dev/null 2>&1 &
    else
        log "Starting tshark on $interface without filters"
        nohup sudo "$TSHARK_PATH" -i "$interface" -w "$output_file" > /dev/null 2>&1 &
    fi
...
stop_tshark() {
    ...
    if [ ${#interfaces[@]} -eq 0 ] && [ ${#services[@]} -eq 0 ]; then
        ...
        sudo pkill -x "$PROCESS_NAME" 2>/dev/null
...
list_services() {
    ...
    sudo ss -tulnp 2>/dev/null | grep -v "pid=" | awk '{print $7}' | cut -d'"' -f2 | sort -u | while read process; do
        if [[ -n "$process" && "$process" != "-" ]]; then
            local ports=$(sudo ss -tulnp 2>/dev/null | grep "\"$process\"" | awk '{print $5}' | awk -F':' '{print $NF}' | sort -u | tr '\n' ' ')

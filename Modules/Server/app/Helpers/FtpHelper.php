...
class FtpHelper
{
    ...
    public function uploadFile (string $remotePath, string $fileContent, string $fileName): void
    {
        $sftp = new \phpseclib3\Net\SFTP($this->server['ip']);
        ...
        if ($fileContent === false) {
            $this->logActivity('sftp-upload-error', 'uploadFile', ['remotePath' => $remotePath]);
            throw new \RuntimeException("Failed to upload remote file: $remotePath");
        }
